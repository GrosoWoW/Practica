...
from UtilesDerivados import delta_frecuencia, cast_frecuencia
import numpy as np
...
class DerivadosSUC(DerivadosAbstracto):
    ...
    def genera_flujos(self):
            ...
            vpv = ("SELECT (ICP1.ICP / ICP0.ICP)/(ICP1.UF / ICP0.UF) AS Cupon FROM "
                   ...
                   "WHERE (Fecha = " + fecha_str(fechaValores) + ")) ICP1 CROSS JOIN ("
                                                           ...
                                                           "WHERE (Fecha = " + fecha_str(fecha_cupon_anterior) + ")) "
                                                                                                                  "ICP0")
            vpv = """SELECT (ICP1.ICP / ICP0.ICP)/(ICP1.UF / ICP0.UF) 
            AS Cupon FROM (SELECT ICP, UF FROM dbAlgebra.dbo.TdIndicadores WHERE 
            (Fecha = @FechaVal)) ICP1 CROSS JOIN 
            (SELECT ICP, UF FROM dbAlgebra.dbo.TdIndicadores WHERE 
            (Fecha = @FechaCupAnt)) ICP0"""

            vpv = vpv.replace("@FechaVal", fecha_str(fechaValores)).replace("@FechaCupAnt", fecha_str(fecha_cupon_anterior))
        ...
        for i in flujos_v:
            ...
            flujos_derivados = flujos_derivados.append(pd.DataFrame([[fechaActual, fondo, 'SUC', id,
        ...
        flujos_nosensibles = flujos_nosensibles.append(pd.DataFrame([[fechaActual, fondo, 'SUC', id,
                                                                      -factor_recibo_fijo, max(fechaActual, fecha_efectiva),
                                                                      moneda,
                                                                      vpv * nocional]],
